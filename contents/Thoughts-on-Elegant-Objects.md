...
## Recap
I still don’t know what I am going to take away from this book as it particularly has strong opinions which do not align with mine. Yet, there are things I have experienced myself and got the same solution. What I found valuable are naming and the rules of thumb as I generally agree on them. Though as always, some sentences are too absolute:
* It’s okay to use return values for mutations: if the only way to get information when performing side-effects, it should better be explicit.
* Sometimes we do not use boolean queries directly within conditions. In those cases it might read far better if we still keep the `is` prefix. And: `isContaining` sounds awful when compared to `contains`.
* Functions have much less overhead than classes. Though composing classes can be easier than composing functions. It depends. But mostly functions.
